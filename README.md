...
- **나와 관련 있는 항목만 노출**: 보조·공유 캘린더에 섞여 들어오는 타인의 일정은 걸러내고, 내가 만들었거나 참석자로 초대된(거절하지 않은) 일정 + 참석자가 없는 개인·공지성 일정만 알람·브리핑에 노출합니다. 메일도 마찬가지로 Gmail 카테고리 중 프로모션·소셜·포럼은 폴링에서 제외해 자동 발송·뉴스레터성 메일이 알람을 울리지 않도록 합니다.
