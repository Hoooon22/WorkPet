...
| 졸음 | 활동 없이 **5분** 경과 (깨우기 후에도 동일) | `sleep` — 눈을 감고 머리 위로 `z Z Z` 표시 |
| 자리비움 | 사용자가 **5분** 이상 입력 없을 때 | `sleep` 자세로 대기 |
| 작업 일시정지 | 트레이 메뉴 "걷기 일시정지" | `sleep` 자세 유지 |
| 새 알림 도착 | Gmail/Calendar 폴링에서 신규 항목 감지 | `alert` — 두 눈이 `!!` 모양으로 변하고 위아래로 통통 튐, 5초간 요약 말풍선 |
| 모든 알림 처리 완료 | 알람 패널을 모두 비웠을 때 | `idle` 복귀 + "모든 알림을 확인했어요 😊" 말풍선 |
| 깨우기 | 졸음/자리비움/걷기 일시정지 상태일 때 펫을 클릭 | `stretch` 모션(약 1.4초) — 클릭은 깨우기로 소비되고 패널은 열리지 않음. 다시 클릭하면 패널이 열림 |
| 더블클릭 | 사용자가 펫을 두 번 클릭 | 빠른 질문 입력창 등장 → Enter로 전송하면 펫이 `think` 모션으로 잠시 생각하다가 Gemini 답변을 말풍선으로 보여줌 (말풍선 클릭으로 닫기, Esc/취소로 입력 취소). Gemini API 키 미설정 시 안내 말풍선이 뜨고, 말풍선을 클릭하면 패널 설정 탭의 키 입력 칸으로 바로 이동 |
| 아침 인사 | 오전 6~11시 첫 등장 | "좋은 아침이에요! ☀️" 말풍선 |
| 복귀 인사 | 자리비움 후 돌아왔을 때 | 시각·일정에 따라 "맛있는 점심 드셨어요? 🍱" / "회의 잘 마치셨어요? 📋" / "어디 갔다 왔어요? 👋" |
| 가챠 획득 | 새 펫을 뽑았을 때 | "안녕! 나는 ○○이야 🎉" 말풍선 |
| 등장 인사 | 트레이 "펫 보이기"로 다시 불렀을 때 | `wave` 모션 + "다시 만나서 반가워요! ✨" 말풍선 |
| 퇴장 인사 | 트레이 "펫 숨기기"로 보낼 때 | `wave` 모션 + "다음에 또 봐요! 👋" 말풍선 후 사라짐 |
| 로그인 안내 | Google 로그인이 성공한 직후 | 인사 말풍선이 끝나면 이어서 "💬 더블클릭하면 뭐든 물어볼 수 있어요" 힌트 말풍선이 표시됨 |

이외에도 **드래그 이동**(원하는 위치로 직접 옮기기, 다른 모니터로 옮기면 해당 모니터를 새 홈으로 인식)과 **3단계 크기**(작게/보통/크게, 트레이에서 변경)를 지원하며, 종료 후에도 마지막 위치가 복원됩니다.
...
├── index.html / panel.html / gacha.html / screenshot.html / team_room.html  # Vite 다중 엔트리
├── vite.config.ts
└── package.json
```

각 창(`pet`, `panel`, `gacha`, `screenshot`, `color-picker`, `team_room`)은 독립된 Vite 엔트리이며, 상태는 `tauri-plugin-store`와 Tauri 이벤트 버스(`orbit:*` 토픽)로 동기화됩니다. 색상 추출 오버레이는 별도 HTML 파일이 아니라 `screenshot.html`을 `color-picker` 창 ID로 재사용합니다.
