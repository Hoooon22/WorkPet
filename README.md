...
### 🔔 알림 & 브리핑
- **자동 폴링**: 30초마다 Gmail 신규 메일(History API 기반 델타 감지)과 Google Calendar 신규/임박 일정을 확인합니다.
...
- **🪟 위젯 모드** (트레이 토글): ON으로 두면 펫이 평소엔 숨어 있다가 알림(브리핑·정기/휴식 알림)이 올 때만 트레이 근처로 톡 튀어나와 말풍선을 띄우고 자동으로 사라집니다. 메시지 길이에 따라 6~15초간 머무르며, 펫·말풍선을 클릭하거나 더블클릭하면 즉시 닫힙니다. 동시에 여러 알림이 도착하면 차례로 보여줍니다(최대 5건). 트레이 위치는 macOS는 메뉴바 우측, Windows는 작업표시줄 모서리를 기준으로 근사합니다(트레이 아이콘 좌표는 OS가 공개 API로 제공하지 않음). 모드를 끄면 펫은 원래 자리로 돌아옵니다.
